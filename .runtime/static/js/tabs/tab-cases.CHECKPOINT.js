...
function switchCaseTab(tab) {
  ...
  tabs.forEach(t => {
    ...
    if (pane) {
      ...
      if (active && t === 'actions') pane.classList.add('flex');
      else if (!active && t === 'actions') pane.classList.remove('flex');
    }
  });
...
async function openCaseModal(caseId) {
  ...
  try {
    ...
    // Actions section (separate div)
    const actionsDiv = document.getElementById('caseModalActions');
    if (actionsDiv) {
      actionsDiv.innerHTML = `
        ${c.status !== 'resolved' && c.status !== 'closed' && c.status !== 'dismissed' ? `
          <button onclick="caseAction('${c.id}', 'investigate')" class="px-4 py-2 rounded-xl bg-yellow-50 text-yellow-600 text-xs font-semibold hover:bg-yellow-100 border border-yellow-200 transition"><i class="fas fa-magnifying-glass mr-1.5"></i>Investigate</button>
          <button onclick="caseAction('${c.id}', 'escalate')" class="px-4 py-2 rounded-xl bg-orange-50 text-orange-600 text-xs font-semibold hover:bg-orange-100 border border-orange-200 transition"><i class="fas fa-arrow-up mr-1.5"></i>Escalate</button>
          <button onclick="caseAction('${c.id}', 'resolve')" class="px-4 py-2 rounded-xl bg-green-50 text-green-600 text-xs font-semibold hover:bg-green-100 border border-green-200 transition"><i class="fas fa-check mr-1.5"></i>Resolve</button>
        ` : `<span class="text-xs text-gray-400 italic py-2">Case is ${c.status} — no further actions available.</span>`}
        ${!c.sar_number ? `
          <button onclick="caseAction('${c.id}', 'file-sar')" class="px-4 py-2 rounded-xl bg-red-50 text-red-600 text-xs font-semibold hover:bg-red-100 border border-red-200 transition"><i class="fas fa-file-alt mr-1.5"></i>File SAR</button>
        ` : `
          <button onclick="downloadSAR('${c.id}')" class="px-4 py-2 rounded-xl bg-emerald-50 text-emerald-600 text-xs font-semibold hover:bg-emerald-100 border border-emerald-200 transition"><i class="fas fa-download mr-1.5"></i>Download SAR Report</button>
        `}
        <button onclick="promptAssign('${c.id}')" class="px-4 py-2 rounded-xl bg-blue-50 text-blue-600 text-xs font-semibold hover:bg-blue-100 border border-blue-200 transition"><i class="fas fa-user-plus mr-1.5"></i>Assign</button>
        <button onclick="promptFindings('${c.id}')" class="px-4 py-2 rounded-xl bg-purple-50 text-purple-600 text-xs font-semibold hover:bg-purple-100 border border-purple-200 transition"><i class="fas fa-pen mr-1.5"></i>Add Findings</button>
      `;
    }
