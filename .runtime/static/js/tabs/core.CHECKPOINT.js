...
function configureUIForRole() {
  const tabAccess = {
    admin:         ['dashboard', 'approvals', 'aiml', 'compliance', 'cases', 'aml', 'security', 'cards', 'admin', 'graph', 'mlops', 'corridors'],
    operator:      ['dashboard', 'payments', 'beneficiaries', 'approvals', 'security', 'cards', 'corridors'],
    compliance:    ['dashboard', 'approvals', 'compliance', 'cases', 'aml', 'aiml', 'security', 'graph', 'mlops', 'corridors'],
    client:        ['dashboard', 'payments', 'beneficiaries', 'cards', 'documents', 'spending360', 'security', 'aiml', 'defi'],
    auditor:       ['dashboard', 'compliance', 'cases', 'aml', 'security', 'graph', 'admin', 'corridors'],
    datascientist: ['dashboard', 'aiml', 'mlops', 'compliance', 'aml', 'graph', 'corridors'],
  };
...
function switchTab(tab) {
  ...
  if (tab === 'payments') loadPaymentForm();
  if (tab === 'aiml') {
  ...
  if (tab === 'corridors') { loadCorridors(); refreshCorridorMap(); }
  ...
  if (tab === 'cards') { loadCards(); if (ROLE === 'admin') loadCardRequests(); }
...
async function loadDashboard() {
  try {
    ...
    document.getElementById('kpiLiquidity').textContent = '$' + Number(data.nostro_liquidity_usd || 0).toLocaleString('en-US', {minimumFractionDigits: 0, maximumFractionDigits: 0});
