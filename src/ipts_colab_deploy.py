...
# Kill any existing processes on ports 8545 and 5001
for port in [8545, 5001]:
    try:
...
# Start Flask
print("  Starting Flask API on port 5001...", end=" ", flush=True)
...
for i in range(10):
    try:
        urllib.request.urlopen("http://127.0.0.1:5001/api/login", timeout=2)
        flask_ready = True
        break
    except urllib.error.URLError as e:
        # If we get HTTP 405 (Method Not Allowed for GET /api/login), the server is UP!
...
print(f"""
  ...
  FLASK API:     http://127.0.0.1:5001/api/
